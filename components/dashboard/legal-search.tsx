...
import { Search, X, Loader2, Filter, ShieldAlert } from 'lucide-react'
...
export function LegalSearch() {
    ...
    const updateParams = (key: string, value: string) => {
        ...
        startTransition(() => {
            router.push(`/dashboard/legal?${params.toString()}`)
        })
    }
    ...
    const hasFilters = search || type !== 'all' || expiring
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center bg-muted/30 p-4 rounded-xl border mb-6">
            ...
            <form onSubmit={handleSearch} className="relative flex-1">
                ...
                <Input
                    placeholder="Buscar empresa, representante o NIT..."
                    value={search}
            ...
            <div className="flex flex-wrap items-center gap-3">
                <Button
                    ...
                    onClick={() => updateParams('expiring', expiring ? 'false' : 'true')}
                    className={cn("h-10 px-4 rounded-lg font-medium text-xs border border-dashed", expiring ? "bg-amber-600 hover:bg-amber-700 border-amber-600 text-white" : "border-amber-300 text-amber-600 hover:bg-amber-50")}
                >
                ...
                <Select value={type} onValueChange={(value) => updateParams('type', value)}>
                    ...
                    <SelectContent>
                        ...
                        <SelectItem value="juridica" className="text-xs">Jurídica (Empresa)</SelectItem>
