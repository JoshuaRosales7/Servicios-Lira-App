    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-foreground">Bitácora & Notas</h1>
            <p className="text-sm text-muted-foreground">Seguimiento y cronología de observaciones por cliente.</p>
          </div>
        </div>
      ...
      {pinnedNotes.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-foreground">Notas Prioritarias</h2>
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {pinnedNotes.map((note) => (
              <Link key={note.id} href={`/dashboard/clients/${note.client_id}`}>
                <div className="h-full bg-card rounded-xl border-l-3 border-l-blue-500 border border-border p-4 hover:border-slate-300 dark:hover:border-slate-700 cursor-pointer transition-colors">
                  <p className="font-medium text-sm text-foreground line-clamp-1 mb-0.5">{note.title || 'Informativo'}</p>
                  <p className="text-xs text-muted-foreground mb-2">{note.clients?.commercial_name || note.clients?.legal_name}</p>
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        <div className="px-6 py-4 border-b border-border">
          <h2 className="text-sm font-semibold text-foreground">Listado Cronológico</h2>
          <p className="text-xs text-muted-foreground mt-0.5">Bitácora completa de gestiones y observaciones.</p>
        </div>
              ...
              <Link key={note.id} href={`/dashboard/clients/${note.client_id}`}
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-medium text-sm text-foreground">{note.clients?.commercial_name || note.clients?.legal_name || 'Individual'}</p>
                  ...
                  <p className="text-xs text-muted-foreground line-clamp-2 max-w-3xl">{note.content}</p>
                </div>
